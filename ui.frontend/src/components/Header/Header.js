...
export default class Header extends Component {
  constructor(props) {
    super(props);
    this.state = { isMenuOpen: false,cartItems:this.props.count };
  ...
  get logo() {
    ...
    if (homeLink) {
      logo = (
        <Link className="Logo-link" to={this.homeLink}>
    ...
    return (
      ...
      >
        ...
        <div>
          ...
          <Link to="/content/wknd-spa-react/us/en/home/cart.html" style={{color:"black"}}>
          <i
            className="fa fa-shopping-cart cartIcon position-relative"
            aria-hidden="true"
          >
            <span
              style={{ fontSize: "15px" }}
            ...
            >
             {localStorage.getItem('cart')!==null?JSON.parse(localStorage.getItem('cart')).c.length:""}
            
            </span>
          </i>
          </Link>
        </div>
        <div className="Header-navigation">{this.navigation}</div>
